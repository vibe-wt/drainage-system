...
def create_pipe_endpoint(payload: CreatePipeRequest, db: Session = Depends(get_db)) -> PipeResponse:
    return PipeResponse(data=create_pipe(db, payload))


@router.patch("/{pipe_id}", response_model=PipeResponse)
def update_pipe_endpoint(pipe_id: str, payload: UpdatePipeRequest, db: Session = Depends(get_db)) -> PipeResponse:
    return PipeResponse(data=update_pipe(db, pipe_id, payload))


@router.delete("/{pipe_id}", response_model=MessageResponse)
def delete_pipe_endpoint(pipe_id: str, db: Session = Depends(get_db)) -> MessageResponse:
