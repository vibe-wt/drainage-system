...
export function UserAccessPage() {
  ...
  async function loadUsers() {
    ...
    try {
      const [nextCurrentUser, nextSessions] = await Promise.all([fetchCurrentUser(), fetchMySessions()]);
      const isAdmin = nextCurrentUser.role === "admin";
      const [nextUsers, nextAuditLogs] = isAdmin
        ? await Promise.all([fetchUsers(), fetchAuthAuditLogs().catch(() => [])])
        : [[], []];
    ...
    <section className="stack-page">
      ...
      <div className="card-grid user-access-grid">
        ...
        {currentUser?.role === "admin" ? (
          ...
          <article className="panel stack-panel">
            <strong>用户管理</strong>
            <p className="muted-inline">当前账号不显示管理员工具，只保留个人资料和会话管理。</p>
            <div className="access-readonly-note">
              <strong>需要 admin 权限</strong>
              <span>创建用户、调整用户状态、重置密码和查看认证审计，当前阶段都只对管理员开放。</span>
            </div>
            {feedbackMessage ? <p className="success-text">{feedbackMessage}</p> : null}
            {errorMessage ? <p className="error-text">{errorMessage}</p> : null}
          </article>
